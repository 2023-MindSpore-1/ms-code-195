...
from src.data.my_util import RandomAugment, resample, Resize
...
class ImageNetFinetune:
    ...
    def __init__(self, args, training=True):
        if args.run_modelarts:
            ...
            train_dir = 'src/data/ima_test/aaa'
...
def create_dataset_imagenet(dataset_dir, args, repeat_num=1, training=True):
    ...
    if training:
        ...
        #     vision.Resize(size=(int(args.image_size), int(args.image_size))),
        #     vision.RandomHorizontalFlip(prob=0.5),
        #     # CutOut(pad_size=cutout_size),  # 是一种新的正则化方法。原理是在训练时随机把图片的一部分减掉
        #     vision.HWC2CHW()]
# 测试图像函数和改train
        transform_img = [
            vision.Decode(to_pil=True),  #先转成PIL才能用RandAugmentMC操作
            Resize(args.input_image_size),
            RandomAugment(15),  #magnitudes[step]
            vision.RandomHorizontalFlip(prob=0.5),
            vision.ToTensor(),
...
# train_test = ImageNetFinetune(args, True)
# my_data = train_test.train_dataset.create_dict_iterator()
#
# for i in my_data:
#     print(i['image'])
